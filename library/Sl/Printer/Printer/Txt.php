...
class Txt extends \Sl\Printer\Printer {
        ...
        if (!isset($this->_prepared_items)) {
            ...
            foreach ($this->getAdditionObjects() as $key => $value) {
                ...
                foreach ($value as $k => $item_object) {
                    if (!($item_object instanceof \Sl_Model_Abstract))                        continue;
    ...
    protected function save($save, \Sl\Printer\Template $template, array $data, array $templ_data) {
       ...
       // error_log(E_ALL); 
        if(!($template instanceof \Sl\Printer\Template\Txt)) {
            throw new \Exception('Only Template\Txt supported.');
        }
