...
class Xls extends \Sl\Printer\Printer {
    ...
    protected function _prepareObjectTemplates() {
        ...
        foreach ($this->_prepareItems() as $key => $value) {
            $template_data+=array_map(function($el) {
                        return '%' . (isset($el['relation']) ? 'L.' : '') . strtoupper($el['name']) . '%';
                    }, $value);
        }
    ...
    protected function save($save, \Sl\Printer\Template $template, array $data, array $templ_data) {
        ...
        if (!($template instanceof \Sl\Printer\Template\Xls)) {
            throw new \Exception('Only Template\Xls supported.');
        }
        /* @var $template \Sl\Printer\Template\Xls */
